...
void solve(){
    ...
    perm.push_back(size - 1); // largest first

    // add non-powers-of-2 in increasing order (except largest)
    for(int i = 0; i < size - 1; i++){
        if(!isPowerOf2(i)) perm.push_back(i);
    }

    // add powers-of-2 in increasing order except largest
